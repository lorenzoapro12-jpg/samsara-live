...
| `indicators` | `fetch_macro.py` (4h/1h/1d) | module réutilisé, testé — ⚠️ **RSI=Wilder + EMA amorcées par SMA depuis le 12/09/2026** : la version antérieure affichait un RSI « Cutler » (RSI 1d 46,4 au lieu de 55,2, RSI 1h 62,4 au lieu de 48,7) et des EMA amorcées sur le dernier close (EMA50 daily 74 701 au lieu de 73 052 → le signal prix/EMA20 daily était **inversé**). Contrôle de non-régression : comparer `market-data.json` à un calcul indépendant (Wilder) avant de faire confiance au panneau 1j. |
